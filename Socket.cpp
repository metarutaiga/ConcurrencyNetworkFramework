...
void Socket::startup()
{
    ...
    Socket::getsockopt = [](socket_t socket, int level, int optname, void* optval, socklen_t* optlen) { return ::getsockopt(socket, level, optname, (char*)optval, optlen); };
    Socket::listen = [](socket_t socket, int backlog) { return ::listen(socket, backlog); };
    Socket::recv = [](socket_t socket, void* buf, size_t len, int flags) { return ::recv(socket, (char*)buf, len, flags); };
    Socket::recvfrom = [](socket_t socket, void* buf, size_t len, int flags, struct sockaddr* addr, socklen_t* addrlen) { return ::recvfrom(socket, (char*)buf, len, flags, addr, addrlen); };
    Socket::setsockopt = [](socket_t socket, int level, int optname, const void* optval, socklen_t optlen) { return ::setsockopt(socket, level, optname, (char*)optval, optlen); };
    Socket::send = [](socket_t socket, const void* buf, size_t len, int flags, char so_temp[Socket::CORK_SIZE]) { return ::send(socket, (char*)buf, len, flags); };
    Socket::sendto = [](socket_t socket, const void* buf, size_t len, int flags, const struct sockaddr* name, socklen_t namelen) { return ::sendto(socket, (char*)buf, len, flags, name, namelen); };
